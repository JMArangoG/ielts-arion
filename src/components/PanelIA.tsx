...
export default function PanelIA() {
  ...
  return (
    <div className="glass-panel p-stack-lg">
      <h2 className="text-lg font-semibold text-orion-text">Sugerencias con IA (opcional)</h2>
      ...
      <label className="mt-stack-md flex items-start gap-stack-sm text-sm text-orion-text">
        <input
          type="checkbox"
          checked={consiente}
          onChange={(e) => setConsiente(e.target.checked)}
          className="mt-0.5 h-4 w-4 accent-orion-primary"
        />
      ...
      <button
        ...
        onClick={pedir}
        className="mt-stack-md rounded-panel bg-orion-primary px-stack-md py-stack-sm font-semibold text-orion-on-primary disabled:cursor-not-allowed disabled:opacity-50"
      >
        {estado === "cargando" ? "Generando…" : "Pedir sugerencias a la IA"}
      </button>

      {estado === "noconfig" && (
        <p className="mt-stack-md text-sm text-orion-muted" role="status">
          IA no configurada en este entorno. Tu plan heurístico sigue activo sin cambios.
        </p>
      )}
      {estado === "error" && (
        <p className="mt-stack-md text-sm text-orion-danger" role="alert">
          El proveedor no respondió. Inténtalo más tarde; tu plan heurístico permanece.
        </p>
      )}
      {estado === "ok" && (
        <ul className="mt-stack-md list-disc pl-stack-lg text-orion-text">
          {sugerencias.map((s) => (
            <li key={s}>{s}</li>
          ))}
        </ul>
